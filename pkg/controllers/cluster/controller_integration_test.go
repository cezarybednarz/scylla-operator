...
var _ = Describe("Cluster controller", func() {
	...
	Context("Node replace", func() {
		...
		It("replace non seed node", func() {
			...
			// Service IP should appear in ReplaceAddresses rack status
			Eventually(func() (map[string]string, error) {
				if err := testEnv.Refresh(ctx, scylla); err != nil {
					return nil, err
				}

				return scylla.Status.Racks[rack.Name].ReplaceAddresses, nil
			...
			// When new pod is scheduled
			Expect(sstStub.CreatePods(ctx, scylla)).To(Succeed())

			//New service should be created with replace label pointing to old one.
			Eventually(func() (map[string]string, error) {
				if err := testEnv.Refresh(ctx, scylla); err != nil {
					return nil, err
				}
		...
		It("replace seed node", func() {
			...
			//There should be an error event generated about replace failure
