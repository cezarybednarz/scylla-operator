...
func (cc *ClusterReconciler) updateStatus(ctx context.Context, cluster *scyllav1alpha1.ScyllaCluster) error {
	...
	for _, rack := range cluster.Spec.Datacenter.Racks {
		rackStatus := scyllav1alpha1.RackStatus{
			ReplaceAddresses: make(map[string]string),
		}
		...
		if existingStatus, ok := cluster.Status.Racks[rack.Name]; ok {
			for k, v := range existingStatus.ReplaceAddresses {
				rackStatus.ReplaceAddresses[k] = v
			}
		}
		...
		for _, svc := range svcList {
			...
			if replaceAddr, ok := svc.Labels[naming.ReplaceLabel]; ok {
				...
				if replaceAddr == "" {
					rackStatus.ReplaceAddresses[svc.Name] = svc.Spec.ClusterIP
				}
